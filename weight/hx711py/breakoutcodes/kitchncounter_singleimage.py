...
def initialize_scale():
    ...
    hx.reset()
    hx.tare()  # Set the offset to zero
...
def main():
    ...
    stable_readings = []  # List to store stable readings
    stability_threshold = 3  # Define a tolerance range for stability (e.g., ±0.5 grams)
    ...
    try:
        while True:
            ...
            if val > 0:
                ...
                # Check if weight is within the stability threshold of previous stable readings
                if not stable_readings or abs(val - stable_readings[-1]) <= stability_threshold:
                    stable_readings.append(val)
                    # If we have 3 consecutive stable readings, take a picture and store the average
                    if len(stable_readings) >= 3:
                        average_stable_weight = sum(stable_readings) / len(stable_readings)
                        print("Weight stabilized. Taking a picture.")
                        print("Take a picture")

                        # Save the average weight in JSON
                        weight_data = {
                        ...
                        print("Average weight saved to weight_data.json")

                        # Reset the scale to zero
                        hx.tare()  # Reset tare to zero after taking the picture
                        print("Place new ingredient")  # Prompt for next ingredient
                        stable_readings = []  # Clear stable readings after action
                else:
                    stable_readings = [val]  # Reset list if weight is not stable
                ...
                stable_readings = []  # Reset stable readings if scale is zeroed
