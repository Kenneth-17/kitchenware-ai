...
# Function to get weight samples and calculate a stable weight
def get_weight_samples(samples=10, delay=0.1):
    readings = []
    for _ in range(samples):
        reading = hx.get_weight(5)
        readings.append(max(0, reading))  # Ensure no negative values
        time.sleep(delay)
    return readings

# Function to calculate mode-like average within 1g tolerance
...
# Function to measure and log the weight of a single ingredient
def measure_ingredient(display_label):
    samples = get_weight_samples(samples=10, delay=0.1)
    stable_weight = get_mode_average(samples)
    print(f"Stable weight for ingredient: {stable_weight:.2f} grams")

    # Capture image and log weight
    image_number = len(os.listdir(IMAGES_DIR)) + 1
    capture_and_log_image(stable_weight, image_number)
    display_label.config(text=f"Weight logged: {stable_weight:.2f} grams")
    time.sleep(2)
    display_label.config(text="Add next ingredient...")

# Real-time weight display updater
def update_display(display_label):
    while True:
        samples = get_weight_samples(samples=5, delay=0.05)
        current_weight = get_mode_average(samples)
...
def start_gui():
    ...
    # Measure button to capture current ingredient's weight
    measure_button = Button(root, text="Capture Weight", font=("Helvetica", 14),
                            command=lambda: measure_ingredient(display_label))
    measure_button.pack(pady=20)

    # Start real-time display updater in a separate thread
    Thread(target=update_display, args=(display_label,), daemon=True).start()
...
def main():
    ...
    print("Scale ready. Starting real-time GUI...")
    start_gui()
