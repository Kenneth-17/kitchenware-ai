...
# Function to get the ingredient name using Google Vision API
def get_ingredient_name_from_image(image_path):
    ...
    # Process the labels detected by Vision API
    ...
    # Return the most likely ingredient (for simplicity, we return the first label)
    return ingredients[0] if ingredients else None

# Function to capture image and log weight with nutrition data
def capture_and_log_image(weight, image_number, meal_type):
    ...
    # Get ingredient name from the image using Google Vision API
    ingredient_name = get_ingredient_name_from_image(image_filename)
    if not ingredient_name:
        print("Unable to detect ingredient name from image.")
        ...
        if monitoring:
            ...
            if stable_readings and sum(stable_readings) / len(stable_readings) > 0:
                ...
                if last_logged_weight is None or abs(average_stable_weight - last_logged_weight) > stability_threshold:
                    ...
                    capture_and_log_image(average_stable_weight, len(os.listdir(IMAGES_DIR)) + 1, meal_type)
